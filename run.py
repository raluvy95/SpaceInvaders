...
if not os.path.isfile(".env"):
    print("Looks like you're running the bot for first time.")
    print("Before continuing the configuration step, I need your token")
    ...
    while True:
        try:
            if js["message"]:
                print(Fore.RED + f"Your token is invalid! Error code: {js['message']}")
                continue
        except KeyError:
            break
    print(Fore.GREEN + "Your token is valid!")
    prefix = input(Fore.WHITE + "Your bot's prefix (leave empty to default prefix): ")
    if not prefix:
        prefix = "$"
